# 가져온 DeepANC의 과거 CPU 검증 기록 — 2026-09-20

아래는 **통합 전 `tokengeoji/DeepANC`에서 수행한 검증 기록**이다. 현재 `tokengeoji/Deep-ANC`의 전체 테스트 수나 실행 지침이 아니다. 당시 명령과 결과를 출처 보존 목적으로 남긴다. 현재는 [Docker 안내](../docker/README.md)에 따라 Docker 내부에서만 실행하며 호스트 `--install`/venv 절차를 사용하지 않는다. 통합 후 회귀와 기존 Jetson 검증 이력은 [루트 HANDOFF.md](../HANDOFF.md)가 기준이다.
