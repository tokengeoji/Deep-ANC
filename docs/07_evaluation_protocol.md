...
**2026-09-20 사용자 명시 연구 목표:** 단순 고역 감쇠가 아니라 **1 kHz 이상에서 딥러닝 ANC가
충분히 튜닝한 비딥러닝 FxLMS/FxNLMS보다 우수함**을 보여야 한다. 1000–1600 Hz는 초기 구간이며
상한이 아니다. 저역·음성·음악 목표를 희생한 고역 평균 개선도 전체 성공으로 보지 않는다.
동일 원천/세션·P/S·출력 제한·실제 인과 지연으로 비교하고, 초기/전이/수렴 후 결과와 증폭을 남긴다.
기존 `FxLMSController`와 `run_fxlms_offline`는 정규화 업데이트이므로 **FxNLMS**로 해석한다.
이름만 다른 두 기준선으로 세지 않는다. 새 비교 계약과 미확정 합격 조건은
[docs/19](19_high_frequency_comparison.md)를 따른다. 아래 legacy 지표·checkpoint 선택을
그대로 새로운 16 kHz 고역 비교의 판정 기준으로 사용하지 않는다.
