...
이 문서는 **통합 전 `tokengeoji/DeepANC`의 README를 보존한 역사적 자료**다. 아래 `Roka-jsj/DeepANC` 주소, `DeepANC` 경로, Docker image와 Python 환경은 당시 기록이며 현재 저장소의 설치·실행 절차가 아니다. 현재 저장소는 [tokengeoji/Deep-ANC](https://github.com/tokengeoji/Deep-ANC)이며 [프로젝트 README](../README.md)와 [Docker 안내](../docker/README.md)를 따른다. OMAP 16 kHz 오프라인 준비는 [별도 안내](JETSON_SETUP.md)를 사용한다. `scripts/train.py`의 `mix/sph` 학습은 OMAP `deepanc` 및 기존 Jetson `src/deep_anc`의 ANC 학습과 별개다. 아래 과거 명령을 현재 환경 준비 명령으로 실행하지 않는다.
