...
핵심 연구 목표는 **1 kHz 이상에서 충분히 튜닝한 FxLMS/FxNLMS보다 좋은 딥러닝 ANC**다.
1000–1600 Hz는 첫 검증 구간이며 목표 상한이 아니다. 시스템 전체의 저역 목표도 유지한다.
...
현재는 하드웨어 연결과 **새 학습 실행을 보류**하고, 동일 조건의 고역 비교에 필요한
학습 전 준비를 진행한다. 기존 SFANC 실행은 [docs/18](18_sfanc_pretraining.md),
최신 비교·자료·실측 요구는 [docs/19](19_high_frequency_comparison.md)를 따른다.
수집 양식·검수·실측 SFANC/기준선 준비 연결은 [실측 실행 안내](20_measurement_runbook.md)를 따른다.
연속 FIR 교체·출력 제한·추가 지연 민감도와 Orin 구성요소 계산시간 진단도 수행했다.
실제 오디오 연결·종단 지연과 HybridANCNet 대비 감쇠 우위는 아직 검증하지 않았다.
...
pre-roll 측정 코드는 수정했지만 기존 NPZ의 시간축은 미재검증이다([docs/01](01_physics_limits.md)).
