...
이 모델은 첨부 논문이나 기존 GCRN의 재현 구현이 아니다. `scripts/`의 기존 음성 향상 모델과 목적함수가 다르며, 실시간 오디오 입출력 엔진이나 OMAP–Jetson 통신도 구현하지 않는다. CPU 테스트 통과와 모델의 인과성만으로 Jetson 실시간 ANC 성능을 판단할 수 없다.
...
저장소 루트에서 실행한다. Jetson의 NVIDIA PyTorch/CUDA 준비는 [JETSON_SETUP.md](JETSON_SETUP.md)를 따른다. `--device cuda`는 CUDA가 없으면 실패하며 CPU로 조용히 대체하지 않는다.

```bash
# 개발 PC의 합성 데이터 실행 점검
python -m deepanc.train --config configs/anc_train.json \
  --smoke-test --device cpu --output runs/smoke-check

# 실제 녹음으로 첫 1 epoch 실행
python -m deepanc.train --config configs/anc_train.json \
  --manifest datasets/anc/prepared/manifest.jsonl \
  --device cuda --epochs 1 --output runs/anc

# 같은 실행을 총 20 epochs까지 이어서 학습
python -m deepanc.train --config configs/anc_train.json \
  --manifest datasets/anc/prepared/manifest.jsonl \
  --device cuda --epochs 20 --output runs/anc \
  --resume runs/anc/last.pt
```
...
실제 ANC-OFF 동기 녹음 학습, Jetson CUDA 실행, 장치의 추가 지연 및 실시간 음향 성능은 각 환경에서 별도로 확인해야 한다. 이후 작업 순서는 [JETSON_HANDOFF.md](JETSON_HANDOFF.md)에 정리되어 있다.
