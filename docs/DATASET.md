...
저장소 루트에서 실행한다. 두 flag는 실제 수집 조건을 확인했다는 명시적 선언이며, 도구가 녹음 상태를 자동 판정한다는 뜻이 아니다.

```bash
python tools/prepare_recordings.py \
...
Jetson 환경 준비와 CUDA 확인 후 저장소 루트에서 실행한다.

```bash
python -m deepanc.train \
