...
이 저장소에서 재검증한 수치가 아니다. 사용자는 **덕트 구조·스피커는 같고 마이크는 더 좋은 것으로
교체했다**고 확인했다. 음원·감쇠 대역·조건은 사용자가 모른다고 답했고, 제품·REF/ERR 교체 역할도 미확인이다.
마이크가 다른 DSP의 10 dB를 현재 Jetson과 직접 비교하거나 성능 차이의 원인을 단정하지 않는다.
Jetson 마이크도 교체하라는 지시는 없으며 현 구성 유지 조건은 그대로다. **DSP/Jetson 출력 방식은
아직 미정**으로 합산·스피커 공유/개수·크로스오버·듀얼 제어기 런타임 배선을 구현하지 않았다.
...
2026-09-15 준비는 기존 저역 분포와 800–1600 Hz 분포를 0.5 확률로 혼합했다.
그때 PCM inventory의 목표 파워는 **[800,1600]**, ERR/loader smoke 평가는 **[800,1600)**였으며
해당 결과를 새 1000–1600 Hz 결과로 이름만 바꿔 재사용하지 않는다. 2026-09-17 확정된
**1000–1600 Hz를 새 합성 기본주파수 목표로 반영**했고 혼합 확률 0.5·기존 저역 분포는 유지했다.
학습 준비 QA는 설정 대역을 읽어 결과에 `target_band_hz`와 상한 제외를 명시하고, 800Hz 미만과
목표 하한 미만의 에너지도 남긴다. 이는 데이터 진단이며 기존 S 신뢰대역이나 trusted loss를 변경하지 않는다.
합성 bank의 `target_1000_1600`·`guard_800_1000`은 기존 지표에 추가했으며 후보·선택 특징·seed는 같다.
세션 분석의 우선 지표도 `target_1000_1600`으로 명시한다. FFT 에너지는 [1000,1600) 규약이다.
대역 독립 bootstrap RIR은 재생성하지 않았으며 원본 archive·과거 QA/Drive 묶음을 수정하지 않았다.
어느 대역도 FFT 파워 진단만으로 SNR·반복 일관성·측정 S 신뢰대역을 판정하지 않는다.
...
대규모로 돌리는 것만으로 새 목표인 1000–1600 Hz의 학습 문제가 해결되지 않는다.
기존 150–600 Hz S 신뢰대역/손실을 임의로 승격하지 않으며 새 S 검증 후 별도 연구 설정이 필요하다.
...
2026-09-15 CPU 기본 실행의 **linear/test/steady/800–1600 Hz** 비교는 다음과 같다.
이는 보존하는 과거 대역의 결과이며 새 1000–1600 Hz 준비의 검증 수치가 아니다.
...
- DSP의 덕트 구조·스피커는 같지만 마이크는 다르다는 사용자 보고를 보존한다.
  모르는 제품·REF/ERR 역할·음원·대역·조건을 추정하지 않고, 기록이 확보되면 비교 가능성을 검토한다.
  현재의 10 dB 보고는 동일 조건의 Jetson 기준선이 아니다. Jetson 마이크 교체 지시로도 해석하지 않는다.
  Jetson 고역 전담 목표가 저역을 포함한 시스템 전체 do-no-harm·quiet zone 검증을 대체하지 않는다.
  **첫 1–1.6 kHz 범위는 확정**됐으며 관련 준비·회귀를 진행한다. 반면 출력 방식·크로스오버·
  듀얼 제어기 연결은 아직 미정이므로 런타임 배선을 임의로 변경하지 않는다.
