...
> ```
>
> **원인**: [`record_duct.py`](../scripts/data/record_duct.py) 가
...
>
> **오프라인 재정렬로는 게이트를 못 채운다.** `realign_recorded_sessions.py` 표본
> 24세션에서 **16/24 (66.7%)** 가 coh² 0.94 대로 복구된다(전 0.012~0.138 → 후 0.817~0.971,
> 선형 Wiener 하한 −0.23 → **−12.09 dB**). 80세션 환산 약 53세션이라 게이트
> (`≥80세션 그리고 ≥90분`)에 미달한다. **게이트를 낮추지 않으므로 재녹음이 정답이다.**
>
> 그리고 **파인튜닝 val −0.07 dB 정체가 이 데이터의 선형 하한 −0.23 dB 와 정확히 일치한다**
...
> → **진짜 물리 한계는 64–150 Hz 뿐이다** (클린 후에도 S 0.706~0.758, 독립 캡처 간 `|H|`
...
| 항목 | 값 |
|---|---|
| 자동 테스트 | **600개 통과** (`.venv/bin/python -m pytest -q`, 약 14분) |
...
1. **지연·대역 부기의 단일 출처.** `Lead.derive(s_delay, handoff, p_delay)` /
   `BandPlan.resolve(...)` 로만 만들 수 있게 하고(pydantic frozen, 손으로 생성 시
   `TypeError`), 나머지는 **읽기만** 한다.
2. **교차 도메인 불변식 검사기.** P−S 상대 τ 상수성 / `coh²(재생→마이크)` / 플랜트 지문
   일치 / lead 유도값 일치 — 측정·QA·게이트·런타임이 **같은 코드**를 호출해야 한다.
3. **실패 증명 없는 게이트 금지.** 게이트를 열거해 FAIL 시키는 fixture 가 없으면 실패하는
   메타 테스트 (`src/deep_anc/ops/gate_registry.py`, 현재 72개 등록).

### 5.1 지금 막고 있는 것 — 영향 순 (2026-08-06 갱신)

| 순위 | 병목 | 근거 | 조치 | 자원 |
|---|---|---|---|---|
| **1** | **recorded 80세션 시간축 붕괴** — 학습 데이터에 배울 관계가 없다 | §2.1 — coh²(source→ERR) 0.021~0.126 | `record_duct.py` 수정본으로 **전량 재녹음** | 스피커 ~2시간 |
...
| 5b | **64–150 Hz 재현 한계** (클린 후에도 S 0.706~0.758) | §2.3 | 소음/상쇄 스피커 저역 개선 — **진짜 하드웨어 항목** | 하드웨어 |
...
1. **recorded 재녹음** — `record_duct.py` 의 재생/녹음 커서 정렬 수정본으로 80세션을 다시
   딴다. QA 게이트에 `coh²(source→ERR) ≥ 0.6` (150–600 Hz) 이 이미 들어갔다.
   **다른 모든 항목의 선결 조건이다** (§2.1). 지금 데이터로는 어떤 학습 결과도 해석할 수 없다.
...
7. 상쇄 스피커 개선 검토 — 단, 대상은 600 Hz 위가 아니라 **64–150 Hz** 다(§2.3).
...
| # | 발생기 | 증거 | 왜 위험한가 |
|---|---|---|---|
| 1 | 신뢰대역 유도식 `intersect(sp.trusted_band_hz(), duct.realistic_target_band_hz, fs/2)` 가 **5곳에 복붙** | `train/trainer.py:205` · `eval/recorded.py:216` · `scripts/eval/evaluate_offline.py:97` · `scripts/demo/evaluate_session.py:175` · `scripts/demo/render_anc_demo.py:159` | `duct.yaml` 을 바꿔도 5곳이 따라온다는 보장이 없다 |
| 2 | `intersect_frequency_bands` 가 **두 번 정의** | `losses/anc_loss.py:28` · `eval/metrics.py:31` — 입력 처리와 오류 메시지가 다르다 | 같은 이름이 다르게 동작한다 |
| 3 | `configs/eval*.yaml` 3곳의 `trusted_band_hz: [150, 600]` 이 **죽은 설정** | 전수 grep — **어떤 코드도 읽지 않는다** | 권위 있어 보이는데 효과가 없어 다음 사람을 속인다 |
| 4 | `measured_design_ceiling_db: 6.53` 이 **대역 표시 없는 손으로 적은 float** | `train_finetune.yaml:99`, 소비처는 `finetune_readiness.py` 의 `.get()` 한 곳. 아티팩트에서 유도하는 코드도 대조 테스트도 없다 | 요구 대역은 [150,1600](상한 4.77)인데 6.53 은 150–600 값이다. **게다가 독립 재계산은 5.21~5.41 로 또 다르다.** 키를 지우면 `inf` 로 fail-open |
| 5 | source→ERR 지연 궤적이 **두 벌** | `data/timeline.py` (대역제한 GCC-PHAT + robust) vs `dsp/invariants.py` (광대역 argmax + 원시 std) — 같은 세션에서 std **1.8 vs 1107** | **정반대 판정을 낸다.** realign-PASS 세션의 27~44% 를 QA 가 오검출로 기각 |
| 6 | do-no-harm 힌지 마진(`+6.0 dB`)과 G4 임계(`−1.0 dB`)가 **서로를 모른다** | 마진을 정확히 만족하는 모델이 게이트를 옥타브 전 대역에서 8~9 dB 차이로 FAIL (직접 실행 확인) | 힌지는 `|S·y|²/|d|²`, 게이트는 `e/d` — 물리량이 다른데 대조 코드도 테스트도 없다 |
| 7 | 게이트 메타 테스트가 **위양성을 강제하지 않는다** | 모든 게이트에 "발동시키는 fixture" 는 있지만 "정상 데이터에서 발동하지 않음" 짝이 없다 | 워치독의 반응이 전부 mute(= 상쇄 0 dB = 최악값)라 오발동이 곧 성능 0 이다 |
| 8 | `diagnose_training_overfit.py --nmse-only` 가 **더 이상 NMSE 를 분리하지 못한다** | `lambda_mrstft/pow/clip` 만 0 으로 만드는데 새 항 `dnh 0.12 / frame 0.5 / sat 1.0` 이 그대로 켜져 있다 | 진단 스크립트가 조용히 거짓말한다 (참조처 0 이라 학습 경로는 무사) |
| 9 | `build_engine` 이 handoff 를 `duct` cfg 에서 **다시 읽는다** | `realtime/engines.py:385-393` | handoff 의 두 번째 유도 |
...
| `measured_design_ceiling_db` 6.53 | — | 독립 재계산이 5.21~5.41 로 불일치 (§5.4 #4) |
