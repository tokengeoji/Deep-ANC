...
> [!IMPORTANT]
> 2026-08-31 immutable raw 재감사에서 이 snapshot의 “2--8 kHz 15--22 dB 증폭”은
> 해당 대역에 source가 없던 unqualified ratio를 정식 ANC 성능처럼 부른 것으로
> 확인됐다. 그 수치는 감쇠/증폭 성능 근거가 아니다. 그러나 ANC ON control이 4/8 kHz
> 에너지를 직접 만들고 ERR도 증가시킨 유해 출력 결함은 별도로 확인됐다. 현행 판정은
> [docs/66](66_20260831_legacy_pretrain_raw_audit.md)을 우선한다.
...
> | "1 kHz 위는 변하지 않는다 / 증폭하지 않는 것이 성공 기준" | source-valid 1--1.6 kHz는 거의 무반응이고, source 없는 4/8 kHz에 ANC control 에너지를 주입 ([§4.7](#47-실기-anc-사전학습-tiny--ort-cpu)) |
...
> ③ 그 위에서 학습한 모델은 source 없는 고역에 **큰 control 에너지를 주입**하고 있었다
>
...
> [!CAUTION]
> **대역 밖에 유해한 control 에너지를 주입한다 — 절대 목표 1 정면 위반이다.**
> 위 4줄은 trusted 대역 발췌였고, 같은 세션 `results/session_20260804_0939/metrics.csv` 에
> 기록된 옥타브밴드는 다음과 같다. 아래 표는 당시 계산값을 보존한 것이며 source-energy
> gate가 없으므로 **음수를 정식 증폭량으로 사용하면 안 된다**:
>
...
>
> ¹ `results/session_20260804_125538/metrics.csv` — README §1.4 그림의 세션. xrun 87로
> 성능 실험은 invalid다.
>
> **손실에는 대역 밖 do-no-harm 항이 없었고 게이트에만 있었다.** source-valid 재계산은
> 250--500 Hz에서 수 dB, 1--1.6 kHz에서 `-4.67--+0.36 dB`의 불안정/무반응이며,
> 2/4/8 kHz 성능은 판단할 수 없다. 한편 300 Hz 자극에서 digital control의 4/8 kHz
> 에너지가 각각 약 -44.7/-40.8 dBFS였고 ERR ON이 OFF보다 18.0/21.6 dB 커진 것은
> 유해 출력 진단으로 유효하다. 상세 재계산은 [docs/66](66_20260831_legacy_pretrain_raw_audit.md)이다.
>
> ```bash
> .venv/bin/python -c "
> import csv
...
| **2** | **source 없는 고역에 control 에너지를 주입하고, 1--1.6 kHz source-valid 성능도 거의 없음** (절대 목표 1 위반) | §4.7, docs/66 | 새 strict P/S + scratch pretrain + 손실 do-no-harm 항 **λ 교정 + 게이트 임계와 대조** | 분석 + 재학습 |
...
2. **손실 do-no-harm 항 완성** — 항 자체는 들어갔지만 λ 가 새 대역 구성에서 재교정되지
   ...
   (§5.4). source-valid 1--1.6 kHz는 거의 무반응이고 source 없는 4/8 kHz에 큰 control
   에너지를 주입한다(§4.7, docs/66). 절대 목표 1 위반이며 잘못된 legacy init을 그대로
   파인튜닝해서 해결할 문제가 아니다.
...
> λ_dnh 는 legacy 0.12 → 0.001을 거쳐 **0.00075**로 후보 재교정했지만, 과거
> `0.264`는 strict-S라도 `loss_start_sample=0`으로 계산한 수치라 실제 Trainer
> 목적함수의 증거가 아니다. strict S + 실제 3549-sample 절단 고정 fixture는
> `0.130`이며 이 또한 실행 출력 분포를 대표하지 않는다. 따라서 canonical 전에는
> `ANCLoss.gradient_budget`에 동일 절단을 전달하고, 실제 A100 모델/배치의
> 0.2–0.4 evidence를 campaign prerequisite ledger에 결속해야 한다.
