...
def exact_condition_audit_v5(
    ...
    """fit_a/fit_b actual PCM의 exact periodic joint Gram 끝 고유값을 계산한다."""
...
def _clock_waveform_rows_v5(plan: Mapping[str, Any], path: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    holdout = next(
        row for row in layout if row.get("path") == path and row.get("role") == "holdout"
    )
    validation = [
        {"name": "holdout", "start": int(holdout["central_start_frame"]), "role": "holdout"},
        {"name": "tail", "start": int(tail["start_frame"]) + PERIOD, "role": "tail"},
    ]
...
def _waveform_transfer_bank_v5(
    *,
    ...
    adc_index = np.arange(len(captured_adc), dtype=np.float64)
    if interpolators is None:
        interpolators = [
            CubicSpline(adc_index, captured_adc[:, mic], extrapolate=False) for mic in range(2)
        ]
    ...
    for row in rows:
        ...
        if adc_q[0] < 0.0 or adc_q[-1] > len(captured_adc) - 1:
            raise ValueError("candidate q가 captured raw 범위를 벗어납니다")
        captured_period = np.column_stack([interpolator(adc_q) for interpolator in interpolators])
...
def estimate_common_clock_from_waveforms_v5(
    *,
    ...
    submitted = np.asarray(submitted_pcm)
    captured = np.asarray(captured_adc_pcm, dtype=np.float64)
