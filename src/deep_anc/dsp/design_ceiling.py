...
배선을 고쳐 P/S 를 다시 측정하면 아티팩트 sha 는 바뀌지만 당시 설정에
적어 둔 4.58은 그대로 남아 계속 통과했다. 4.58은 2026-08-05 legacy
diagnostic 사고를 설명하는 역사적 수치이며 현재 strict P/S의 상한이 아니다.
...
def worst_octave_ceiling_db(
    ...
    *,
    ...
    그런데 진입 게이트는 대역평균 상한(에너지가중)만 보고 있었다. 두 값은
    현행 strict P/S에서도 다르다. ``configs/duct.yaml``이 가리키는 같은
    capture의 P/S와 ``PlantDelays.lead()``가 유도한 timing으로 다시 풀면::

        전대역 [150, 1600]  +26.10 dB
        최악 옥타브 125 Hz  +17.28 dB

    이 수치는 모델의 실측 감쇠가 아니라 인과 FIR 설계 상한이다. 또한 NPZ가
    바뀌면 다시 유도해야 하며 코드에 timing 숫자를 고정하지 않는다. 2026-08-05
    P1602/S1462/lead116의 4.827/2.159 dB 예시는 provenance가 부족한
    legacy diagnostic-only이며 current official 근거로 사용하지 않는다.

    평균이 최악값을 가리는 구조는 여전히 있으므로, 게이트는 두 값 중 작은
    옥타브 상한을 사용한다.
    """
