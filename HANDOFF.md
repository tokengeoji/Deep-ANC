...
- 감쇠 기준은 ERR **한 점**, 고주파 정의는 **1 kHz 이상**이다. 2026-09-17 사용자는
  ...
  **Jetson의 우선 대역은 1–1.6 kHz로 확정했다.** 새 준비 설정·지표에 1000–1600 Hz를 반영했고,
  2026-09-15의 800–1600 Hz 설정·실험 결과는 과거 비교 기록으로 보존한다.
...
### 2026-09-17 DSP 실험 조건 확인·미확정 연결 조건

사용자는 `omap138` DSP(사용자 표기)의 FxNLMS로 **10 dB 이상 감쇠**했지만
**1 kHz 이상을 잘 잡지 못한다**고 보고했다. 이는 사용자 보고이며 이 저장소에서 재검증한
성능 수치가 아니다. 추가로 **DSP 실험의 덕트 구조와 스피커는 같고, 마이크는 더 좋은 것으로
교체했다**고 확인했다. "더 좋은"은 사용자 설명이며 제품·주파수응답 등의 정량 검증이 아니다.
음원·감쇠 대역·측정 조건은 사용자가 모른다고 답했다. 마이크 제품과 REF/ERR 중 어느 마이크를
교체했는지도 확인되지 않았다. 마이크가 다른 실험의 10 dB를 현 Jetson 구성과
직접 비교하거나, 차이의 원인을 마이크·알고리즘·연산 장치 중 하나로 단정하지 않는다.
**Jetson 마이크 교체 지시는 받지 않았다.** 현 Jetson 마이크·USB DAC 유지 조건은 그대로다.
Jetson의 1–1.6 kHz 보완 목표를 시스템 전체 대역의 성공 판정과 구분한다.

확정 사항과 남은 설계/자료 항목은 다음과 같다.

1. **확정:** Jetson 우선 대역 1–1.6 kHz, DSP 실험의 동일 덕트 구조·스피커 및 다른 마이크.
2. **미정:** DSP/Jetson 출력 합산·분리, 스피커 공유 여부와 실제 제어 스피커 수.
   DSP 실험에서 스피커가 같았다는 답변을 향후 두 출력의 공통 스피커 사용 승인으로 해석하지 않는다.
3. **자료 미확인:** DSP의 마이크 제품·REF/ERR 역할·음원·감쇠 대역·구동/측정 조건.
   관련 기록을 확보하면 비교 가능성을 검토하되, 사용자가 모르는 정보를 추정해 채우지 않는다.

출력 구성이 확정되기 전에는 듀얼 제어기 합산·크로스오버·출력 라우팅·공유 ERR 적응 규칙과 런타임 배선을
...
- MIMII 정책 반영 시점에는 기존 800–1600 Hz 생성 설정·S 신뢰대역·손실 대역을 변경하지 않았다.
  이후 확정된 목표에 맞춰 `synthetic_target_band_hz=[1000,1600]`만 갱신하고 혼합 확률 0.5와
  저역 기본 분포를 유지했다. 새 목표를 S 신뢰대역 확대나 고역 손실 검증 완료로 해석하지 않는다.
  이번에는 원본 재다운로드·Drive 변경·실제 corpus 학습·오디오 출력을 하지 않았다.
- 학습 준비 QA는 생성 설정의 대역을 읽고 결과에 경계를 명시한다. 세션 분석은 기존
  `target_1000_1600`을 우선 지표로 선언하며 저역·800–1000Hz·전체 대역을 함께 남긴다.
  필터 뱅크에는 `target_1000_1600`·`guard_800_1000`을 추가했지만 후보·특징·seed는 바꾸지 않았다.
  FFT 에너지 지표는 1000Hz 포함/1600Hz 제외다. ESS CLI 기본도 1000–1600Hz로 갱신했고
  과거 대역은 `--band 800 1600`으로 명시할 수 있다. ESS pair/위상 진단과 FFT 에너지 지표는 다른 검사다.
...
정리했다. 중단된 실행이 남긴 containerd 고아 lease 하나도 컨테이너·태스크·이미지 참조가 없음을
확인한 뒤 공식 lease 삭제와 GC로 회수했다. 최종 루트 여유 공간은 12GB이고 Docker 이미지·컨테이너·
볼륨·빌드 캐시는 없다. 저장소의 기본 베이스는 바꾸지 않았다. 임시 이미지 ID는
...
### 최신 CPU 검증 결과 — 2026-09-17 1–1.6 kHz 준비 반영

- Docker 전체 회귀 `.venv/bin/python -m pytest -q -o addopts= -ra`:
  **1190 passed, 2 skipped (65.23초)**. skip은 현장 raw 진단 파일·실측 `metrics.md` 부재다.
- 준비 QA·경로 진단·세션 분석 관련 208개와 필터 뱅크 57개 집중 검사를 통과했다.
  모두 장치 없는 합성 회귀이며 실제 corpus 학습이나 DSP/Jetson 감쇠 검증이 아니다.
- `pip check`, `git diff --check` 통과. `--require-band 1000 1600 --require-broadband --json`은
  예상대로 exit 1이다. 고역 S 신뢰대역 미달·광대역 도착 조건 미달을 그대로 기록했다.
- 원본 다운로드·Drive 변경·오디오 출력·실제 GPU 학습은 하지 않았다. 출력 합산 구성은 계속 미정이다.

### 이전 CPU 검증 결과 — 2026-09-17 MIMII 정책 반영 시점
...
2026-09-17 첫 검증 범위는 **1000–1600 Hz로 확정**됐다. 같은 대역으로 준비를 발전시키더라도
기존 exit 1 기록이나 S의 150–600 Hz 신뢰대역을 통과 상태로 바꾸지 않는다.
...
1. **이 PC에서 계속**: 목표 대역 지표·ESS 반복 진단·계수 전달 연구 API·다중 seed 필터 뱅크와
   과거 REF 특징 선택 기준선과 MIMII train-only 정책, 1000–1600 Hz 준비 설정·지표는 반영했다. 다음은
   ...
   DSP/Jetson 출력 구성은 아직 미정이므로 듀얼 제어기 연결·크로스오버를 구현하지 않는다.
   DSP의 마이크 차이와 미확인 실험 조건을 별도로 추적하며 10 dB 보고를 동일 조건 기준선으로 쓰지 않는다.
