...
class PreflightRequirementsTest(unittest.TestCase):
    ...
    def test_container_fallback_requires_real_arm64_jetson_gpu_compute(self):
        pytorch = {"cuda_compute_ok": True, "device_name": "Orin"}
        detected, evidence = PREFLIGHT.jetson_identity("aarch64", None, None, pytorch, "jetson")
        self.assertTrue(detected)
        self.assertEqual(evidence, "arm64_managed_jetson_container_and_verified_orin_tegra_cuda")
        for architecture, marker, cuda_ok, name in (
            ("x86_64", "jetson", True, "Orin"),
            ("aarch64", "cpu", True, "Orin"),
            ("aarch64", None, True, "Orin"),
            ("aarch64", "jetson", False, "Orin"),
            ("aarch64", "jetson", True, "NVIDIA A100"),
        ):
            with self.subTest(architecture=architecture, marker=marker, cuda_ok=cuda_ok, name=name):
                detected, _ = PREFLIGHT.jetson_identity(
                    architecture, None, None, {"cuda_compute_ok": cuda_ok, "device_name": name}, marker
                )
                self.assertFalse(detected)

    def test_visible_hardware_evidence_is_preserved(self):
        detected, evidence = PREFLIGHT.jetson_identity("aarch64", "NVIDIA Jetson AGX Orin", None, {}, None)
        self.assertTrue(detected)
        self.assertEqual(evidence, "device_tree_model")
        detected, evidence = PREFLIGHT.jetson_identity("aarch64", None, "# R36", {}, None)
        self.assertTrue(detected)
        self.assertEqual(evidence, "l4t_release_file")

    def test_dependency_error_uses_existing_docker_installation_contract(self):
        report = healthy_report()
        report["dependencies"]["scipy"] = {"ok": False, "error": "missing"}
        errors, _ = PREFLIGHT.evaluate_report(report)
        self.assertTrue(any("docker/README.md" in error and "bootstrap_jetson_local.sh" in error for error in errors))
        self.assertFalse(any("Install requirements-jetson.txt" in error for error in errors))

    def test_hidden_host_metadata_is_not_reported_as_measured_jetpack(self):
        report = healthy_report()
        report["system"]["jetpack_package_version"] = None
        report["system"]["l4t_release"] = None
        errors, warnings = PREFLIGHT.evaluate_report(report, require_cuda=True, require_jetson=True)
        self.assertEqual(errors, [])
        self.assertTrue(any("does not establish its JetPack version" in warning for warning in warnings))
