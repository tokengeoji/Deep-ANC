...
def jetson_identity(architecture, model, l4t_release, pytorch, container_kind):
    """Docker에서 호스트 메타데이터가 숨겨져도 실제 Orin GPU 증거를 확인한다."""
    if architecture not in ("aarch64", "arm64"):
        return False, "non_arm64"
    if "jetson" in (model or "").lower():
        return True, "device_tree_model"
    if l4t_release is not None:
        return True, "l4t_release_file"
    gpu_name = pytorch.get("device_name", "").lower()
    if (container_kind == "jetson" and pytorch.get("cuda_compute_ok", False)
            and any(name in gpu_name for name in ("orin", "tegra"))):
        return True, "arm64_managed_jetson_container_and_verified_orin_tegra_cuda"
    return False, "insufficient_hardware_evidence"
...
def evaluate_report(report, require_cuda=False, require_jetson=False):
    ...
    for name, dependency in report["dependencies"].items():
        if not dependency["ok"]:
            errors.append(
                "{} cannot be imported: {}. Restore the existing Docker image/venv "
                "using docker/README.md; jetson-local dependencies are managed by "
                "scripts/docker/bootstrap_jetson_local.sh. Keep the project's pinned "
                "requirements and NVIDIA PyTorch.".format(name, dependency["error"])
            )
    pytorch = report["torch"]
    if not pytorch["import_ok"]:
        errors.append(
            "PyTorch cannot be imported: {}. Restore the project's Docker image/venv "
            "using docker/README.md and preserve its pinned NVIDIA build. "
            "Compatibility references: {} and {}.".format(
    ...
    if pytorch["import_ok"]:
        ...
        if missing_apis:
            errors.append(
                ...
                "these APIs (upstream PyTorch 1.13+). Restore the project's Docker image/venv "
                "via docker/README.md, preserving its JetPack-compatible NVIDIA pins. "
                "Compatibility reference: {}. No packages were upgraded.".format(
    ...
    if report["system"]["is_jetson"] and report["system"]["jetpack_package_version"] is None:
        if report["system"].get("l4t_release"):
            warnings.append("nvidia-jetpack metapackage was not found; identify JetPack from the recorded L4T release.")
        else:
            warnings.append(
                "Host JetPack/L4T metadata is not visible in Docker. Verify the host's "
                "installation record; the validated GPU identity does not establish its JetPack version."
            )
...
def collect_report():
    ...
    pytorch = torch_info()
    container_kind = os.environ.get("DEEP_ANC_CONTAINER")
    is_jetson, detection_basis = jetson_identity(architecture, model, l4t, pytorch, container_kind)
    return {
        "schema_version": 1,
        ...
        "system": {
            ...
            "is_jetson": is_jetson,
            "jetson_detection_basis": detection_basis,
