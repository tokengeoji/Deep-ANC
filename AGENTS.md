...
3. **스피커 연결 시간을 최소화하라 (2026-08-05 사용자 지시).**
   ...
   **사용자 입회 요건 완화 (2026-09-03 사용자 명시 지시).**
   > "실측이 필요하면 언제든 해도 좋다 사용자가 없어도 실측해도 되니까 해당 규정도 바꿔라" /
   > (위험 — 무인 상태에서 문제가 나도 즉시 알아챌 사람이 없다는 근거를 제시한 뒤) "바꾸기 원해"
   따라서 **사용자 물리적 입회는 더 이상 절대 조건이 아니다.** 다만 이 완화는 "언제든 즉흥적으로
   소리 내도 된다"는 뜻이 아니며, 위 항목들(사전 설계·dry-run·시간 고지·연결 최소화)과 아래
   안전 섹션의 나머지 요건(볼륨 최소, ANC OFF 시작, 기존 안전 감시 로직 의존)은 그대로 유지한다.
   무인 실행이라도 세이프티 감시(포화/RMS/DC/발산 워치독)가 계속 자동으로 무음 처리하는
   경로만 쓰고, 그 감시 범위 밖의 새로운 실험(새 하드웨어 경로, 미검증 진폭 상한 등)은
   여전히 사람에게 먼저 물을 것.
...
evaluate_session, run_realtime)는 **볼륨 최소 상태에서만**. 런타임은 항상 ANC OFF로 시작.
사용자 물리적 입회는 절대 조건이 아니다(2026-09-03, 위 규칙 3의 완화 참조) — 그러나 무인
실행도 사전 dry-run·연결시간 최소화·기존 세이프티 감시 의존 원칙은 그대로 지킨다.
