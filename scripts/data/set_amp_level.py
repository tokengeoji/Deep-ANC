...
#
# ⚠ 프로브나 채널 구성을 바꾸면 이 교정을 다시 해야 한다. 추정하지 말고
#   같은 노브 위치에서 두 측정을 나란히 다시 재라 — 그렇게 하지 않아서
#   이 상수를 오늘 세 번 틀렸다(-44.0 → -48.3 → -50.1).
TARGET_DBFS = -50.1
TOLERANCE_DB = 2.0
...
def measure(args) -> int:
    ...
    probe = sd.rec(
        int(1.5 * FS), samplerate=FS, channels=2, dtype=MEASUREMENT_DTYPE[0], device=in_dev
    )
    sd.wait()
    ...
    def callback(indata, outdata, frames, _time, status):  # noqa: ANN001
        ...
        # 입력 변환을 PortAudio 에 맡기면 풀스케일 규약이 장치마다 달라져
    ...
    stream = sd.Stream(
        samplerate=FS, blocksize=1024, dtype=MEASUREMENT_DTYPE, channels=2,
